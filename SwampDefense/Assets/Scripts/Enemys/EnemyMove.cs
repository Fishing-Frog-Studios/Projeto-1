...
{
    [Header("Referencia")]
    [SerializeField] private Rigidbody2D enemyBox; //caixa de colis�o do inimigo
...
//    private bool flipSprite;   
    ...
    public void ReceberDano(float dano)
    {
        ...
        if (spriteRenderer != null)
        {
            StopAllCoroutines(); // Evita piscar errado se levar v�rios danos seguidos
    ...
    void MoveDirection()
    {
        Vector2 direction = (destiny.position - transform.position).normalized; //defini a dire��o do movimento
        enemyBox.linearVelocity = speed * direction; // aplica a dire��o de movimento e a velocidade no inimigo
    }
    void PointMoviment()
    {
        if (Vector2.Distance(destiny.position, transform.position) <= 0.1f) //compara se a dintancia dos elementos � menor que 0.1
        {
    ...
    void Direction() // atualizar quando tiver sprites e a anima��o
    {
